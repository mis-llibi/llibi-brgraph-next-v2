    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const companyData = await Promise.all(
        companies.map(async (company) => {
          const employees = await tx.maxicareMasterlist.count({
            where: {
              clientId: clientId,
          ...
          const detailedDependents = await Promise.all(
            sortedRelations.map(async (relation) => {
              if (relation.RELATION?.toLowerCase() === "principal") return;
              const count = await tx.maxicareMasterlist.count({
                where: {
                  clientId: clientId,
                  COMPANY: company.COMPANY,
...
const chart3 = async (
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const companyData = await tx.maxicare.groupBy({
    ...
    const combinedData = await prisma.maxicare.groupBy({
      by: ["Claim_Type"],
      _sum: {
        Approved_Claim_Amount: true,
      },
...
const chart4 = async (
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const initialData = await tx.maxicare.groupBy({
        by: ["Relationship"],
        _sum: {
          Approved_Claim_Amount: true,
        },
      ...
      const payload = initialData.map(async (data) => {
        ...
        const { _sum, _count, _avg, ...payload } = data;
        const headcount = await tx.maxicareMasterlist.count({
          where: {
            clientId,
