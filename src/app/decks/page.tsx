...
import UploadDeckWithClientSelect from "@/components/decks/UploadDeckWithClientSelect";
import EditDeck from "@/components/decks/EditDeck";
import ViewDeck from "@/components/decks/ViewDeck";
import LoadingSpinner from "@/components/LoadingSpinner";

interface DeckWithClient {
  id: number;
  name: string;
  description: string;
  key: string;
...
const DecksPage = () => {
  ...
  const { data: decks, isLoading } = useQuery({
    queryKey: ["allDecks"],
    queryFn: async () => {
      const response = await apiClient.get(`/decks/getAllDecks`);
      if (response.status === 200) {
        return response.data.decks as DeckWithClient[];
      } else {
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <div className="text-sm text-gray-600">
            Total Decks: {decks ? decks.length : 0}
          </div>
          <button
        ...
        <tbody>
          {isLoading ? (
            <tr>
              <td
                colSpan={6}
                className="text-center border border-gray-300 px-4 py-8"
              >
                <div className="flex justify-center items-center">
                  <LoadingSpinner size="lg" color="primary" />
                  <span className="ml-2">Loading decks...</span>
                </div>
              </td>
            </tr>
          ) : decks && decks.length > 0 ? (
            decks.map((deck: DeckWithClient) => (
              <tr key={deck.id} className="hover:bg-gray-50">
                <td className="px-4 py-2 border border-gray-300 font-medium">
                  {deck.clients.client_name}
                </td>
            ...
            <tr>
              ...
              >
                <div className="text-gray-500">
                  <div className="text-lg font-medium">No decks found</div>
